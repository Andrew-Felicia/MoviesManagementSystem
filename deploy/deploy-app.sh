...
configured_images="$(docker compose config --images app)"
grep -Fxq "$compose_image" <<< "$configured_images" \
  || fail "The app image '$compose_image' was not found in the configured app images: $configured_images"
